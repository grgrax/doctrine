<?php
class fund_category_report_m extends CI_Model
{
	...
	public $rules=array(
		array(
			'field'=>'name',
			'label'=>'Name',
			'rules'=>'trim|required|is_unique[tbl_fund_categories.name]|min_length[3]|xss_clean',
			),
		array(
			'field'=>'description',
			'label'=>'Description',
	...
	//for reporting
	function most_campaigns_report($limit=0,$offset=0){
		$query="SELECT fund_category_id, COUNT( DISTINCT id ) as count
		FROM tbl_campaign
		GROUP BY fund_category_id
	...
	//for reporting
	function most_donations_report($limit=0,$offset=0){
		$query="SELECT fund_category_id, COUNT( DISTINCT id ) as count
		FROM tbl_campaign
		GROUP BY fund_category_id
		ORDER BY COUNT( DISTINCT id ) desc";
