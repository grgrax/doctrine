				...
				if ($rows && count($rows) > 0) {
					...
					foreach ($rows as $row) {
						...
						switch($row['status']){
							...
							{
								$alertClass="warning";
								if(permission_permit(array('activate-user'))) 
							...
							{
								$alertClass="";
								if(permission_permit(array('block-user'))) 
									$actions[]='block';
								if(permission_permit(array('delete-user'))) 
									$actions[]='delete';
								break;
							}
						}
						...
						<tr class="<?php echo $alertClass?>">
							...
							<td>
								<?php if($row['username']=='superadmin') continue; ?>
								<?php if(permission_permit(array('edit-user'))) { ?>
								<a class="btn btn-sm btn-default btn-icon btn-hover-success add-tooltip fa fa-pencil" 
								data-toggle="tooltip" 
								href="<?= $url ?>edit/<?= $row['username'] ?>" 
								...
								{ 
									...
									?>
									<a href="<?=$url.$action.'/'.$row['username'] ?>" class="btn btn-sm btn-icon add-tooltip <?=$class?>" 
										data-toggle="tooltip" 
